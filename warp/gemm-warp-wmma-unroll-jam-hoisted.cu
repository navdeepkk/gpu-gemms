  ...
  for(int kk = 0; kk < k; kk += Ktile){
	...
	#pragma unroll
		...
		if(kk == 0){
		  ...
		  wmma::load_matrix_sync(c_accum[(i + WM) / WM][j / WN], c_warp_tile_offset + ((i + WM) * Ntile) + j, Ntile, C_LAYOUT);
		}
                // call mma.sync();
		...
		// in chunks of 16x16 each. This loop is meant to run only once per thread block tile.
		if(kk == 0){
		  wmma::load_matrix_sync(c_accum[i / WM][(j + WN) / WN], c_warp_tile_offset + (i * Ntile) + (j + WN), Ntile, C_LAYOUT);
		  wmma::load_matrix_sync(c_accum[(i + WM) / WM][(j + WN) / WN], c_warp_tile_offset + ((i + WM) * Ntile) + (j + WN), Ntile, C_LAYOUT);
		}
                // call mma.sync();
