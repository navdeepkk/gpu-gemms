...
__host__ void init_host_matrices(DTYPEAB *a, DTYPEAB *b, DTYPECD *c, DTYPECD * a_f, DTYPECD * b_f){
  for(int i = 0; i < M; i++){
    for(int j = 0; j < K; j++){
...
__host__ void printMatrixHalf(half * matrix, int m, int n){
  for(int i = 0; i < m; ++i){
    for(int j = 0; j < n; ++j){
...
__global__ void GEMM(DTYPEAB * a, DTYPEAB * b, DTYPECD * c, DTYPECD * d, int m, int n, int k){
  ...
  for(int kk = 0; kk < k; kk += Ktile){
	...
	*(hbase + v) = (*(hbase + v)) + __float2half(1.0f);
    ...
    //      printf("%f ", __half2float(bsmem[i * (Ntile + PADDING_AB) + j]));
    //    }
    //    printf("\n");
    //  }
    //}
    ...
    //      printf("%f ", __half2float(asmem[i * (Ktile) + j]));
    //    }
    //    printf("\n");
    //  }
    //}
    ...
    //  DTYPEAB * gmemBase = a_thread_tile_base_copy + ((i / Ktile) * k) + (i % Ktile);
    //  #pragma unroll
    ...
    //  DTYPEAB * gmemBase = b_thread_tile_base_copy + ((i / Ntile) * n) + (i % Ntile);
    //  #pragma unroll
    //  for(int j = 0, e = ELEMS_PER_THRD_AB; j < e; ++j){
    //    *(smemBase + j) = *(gmemBase + j);
    //  }
...
__global__ void GEMM_NAIVE(DTYPECD * a, DTYPECD * b, DTYPECD * c, int m, int n, int k){
  ...
  for(int kk = 0; kk < k; ++kk){
    if(i_iter < m && j_iter < n){
      temp += a[i_iter * k + kk] * b[kk * n + j_iter];
    }
  }
...
int main(){
  ...
  DTYPECD *d_a_float, *d_b_float, *h_a_float, *h_b_float, *d_c, *d_d, *h_c, *h_c_gpu_res, *d_c_naive;
  int m ,n, k;
  ...
  h_a_float = (DTYPECD*) malloc(m * k * sizeof(DTYPECD));
  h_b_float = (DTYPECD*) malloc(k * n * sizeof(DTYPECD));
  ...
  check_cuda_error(cudaMalloc(&d_a_float, m * k * sizeof(DTYPECD)));
  check_cuda_error(cudaMalloc(&d_b_float, k * n * sizeof(DTYPECD)));
  ...
  init_host_matrices(h_a, h_b, h_c, h_a_float, h_b_float);
  ...
  cout<<"Launching with: ("<<grid.x<<" "<<grid.y<<" "<<grid.z<<"), ("<<block.x<<" "<<block.y<<" "<<block.z<<") \n";
  ...
  #ifdef DEBUG_GPU 
  check_cuda_error(cudaMemcpy(d_a_float, h_a_float, m * k * sizeof(DTYPECD), cudaMemcpyHostToDevice));
  check_cuda_error(cudaMemcpy(d_b_float, h_b_float, k * n * sizeof(DTYPECD), cudaMemcpyHostToDevice));
  
  dim3 block2(NBLOCK, MBLOCK, 1);
  dim3 grid2((n + NBLOCK - 1) / NBLOCK, (m + MBLOCK - 1) / MBLOCK, 1);

  GEMM_NAIVE<<<grid2, block2>>>(d_a_float, d_b_float, d_c_naive, m , n, k);	
