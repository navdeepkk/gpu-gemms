...
__global__ void GEMM(DTYPE * a, DTYPE * b, DTYPE * c, int m, int n, int k){
  ...
  int WarpAchunktocopy = WarpKtile;
  int WarpBchunktocopy = WarpKtile;
  ...
  // Allocate a Ctile in registers of dimensions (Mtile, Ntile).
  DTYPE Cout[Mtile][Ntile] = {0};
  
  // The frags will only hold the data that is only required for computation
  // by this thread. This requirement will be diferrent for afrag and bfrag,
  // because it depends on the number of times a single warp has to iterate
  // to compute the results in that dimension. Rather than allocating
  // (WarpMtile / WarpdimX) * WarpKtile registers, we only allocate WarpKtile
  // registers and then reuse them in different iterations of the corresponding
  // loops.
  DTYPE AFrag[WarpKtile];
  DTYPE BFrag[WarpKtile];
...
#pragma unroll
	...
	for(int kkk = 0; kkk < Ktile; kkk += WarpKtile){
	  if(kkk < k){
	    ...
	    // Copy the warp operands from shared memory to registers. The movement and the allocation
	    // of register tiles happens at the level of threads. Hence only those chunks of data must
	    // be moved in which this thread(within a warp) is going to operate on.
	    #pragma unroll
	    // Afrag resides linearly in the memory in the k dimension hence we can copy it normaly.
	    for(int cpi = 0; cpi < WarpAchunktocopy; ++cpi){
	      AFrag[cpi] = a_warp_tile_offset_compute[cpi]; 
	    }
	    if(blockIdx.x == 0 && blockIdx.y == 0 && warpIdx.x == 0 && warpIdx.y == 0){
	      for(int i = 0; i < WarpAchunktocopy; ++i)
		printf("%f", AFrag[i]);
	      printf("\n");
	    }
	    #pragma unroll
	    // Bfrag does not reside in the memory linearly in the k dimension. hence we multiply it
	    // with Ktile.
	    for(int cpi = 0; cpi < WarpBchunktocopy; ++cpi){
	      BFrag[cpi] = b_warp_tile_offset_compute[cpi * Ktile]; 
	    }
	    __syncwarp();
...
#pragma unroll
		for(int k_iter = 0; k_iter < WarpKtile; ++k_iter){
		  ...
		  // Cout[i_iter * Nchunk + j_iter] += a_thread_tile_offset_compute[i_iter * Ktile + k_iter] *b_thread_tile_offset_compute[k_iter * Ntile + j_iter];
		}	
	      }
	    }
	  }
