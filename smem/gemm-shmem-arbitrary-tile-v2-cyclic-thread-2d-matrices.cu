...
__global__ void GEMM(DTYPE * a, DTYPE * b, DTYPE * c, int m, int n, int k){
  ...
  // TODO: Insert checks here to see if the tile size in elements is less than
  ...
  for(int kk = 0; kk < k; kk += Ktile){
    ...
    // essentially equal to a[i_iter_tile_base][kk].
    // essentially equal to b[kk][j_iter_tile_base].
...
#pragma unroll
	...
	// essetnially equal to c[i_iter_tile_base + i_iter_thread_base][j_iter_tile_base + j_iter_thread_base]
...
#pragma unroll
	for(int k_iter = 0; k_iter < Ktile; ++k_iter){
	  // essentially equal to asmem[i_iter_thread_base][k_iter]
	  // essetnially equal to bsmem[k_iter][j_iter_thread_base]
