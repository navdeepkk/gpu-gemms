...
__global__ void GEMM(DTYPE * a, DTYPE * b, DTYPE * c, int m, int n, int k){
  ...
  __shared__ DTYPE csmem[Mtile * Ntile];
  ...
  constexpr int Cchunktocopy = (Mtile * Ntile) / (MBLOCK * NBLOCK);
  ...
  // Number of threads required to copy one row of B.
  int num_threads_to_copy_one_Crow = Ntile / Cchunktocopy;
  ...
  DTYPE *c_thread_tile_base_copy = c_tb_tile_offset;
  DTYPE *a_thread_tile_base_compute = &asmem[0];
  DTYPE *b_thread_tile_base_compute = &bsmem[0];
  DTYPE *c_thread_tile_base_compute = &csmem[0];

  DTYPE *c_thread_tile_offset_copy;
  ...
  DTYPE *c_thread_tile_offset_compute;
  ...
  for(int kk = 0; kk < k; kk += Ktile){
    ...
    int C_row_to_copy_in_global = linear_tid / num_threads_to_copy_one_Crow;
    int C_col_to_copy_in_global = linear_tid % num_threads_to_copy_one_Crow * Cchunktocopy; 
    ...
    c_thread_tile_offset_copy = c_thread_tile_base_copy + C_row_to_copy_in_global * n + C_col_to_copy_in_global; 
...
#pragma unroll
    for(int cpi = 0; cpi < Cchunktocopy; ++cpi){
      csmem[linear_tid * Cchunktocopy + cpi] = c_thread_tile_offset_copy[cpi]; 
    }
    __syncthreads();
    ...
    // responsible for.
#pragma unroll
    for(int i_iter_thread_base = threadIdx.y; i_iter_thread_base < Mtile; i_iter_thread_base+=MBLOCK){
#pragma unroll
      for(int j_iter_thread_base = threadIdx.x; j_iter_thread_base < Ntile; j_iter_thread_base+=NBLOCK){
        c_thread_tile_offset_compute = c_thread_tile_base_compute + i_iter_thread_base * Ntile + j_iter_thread_base;
...
#pragma unroll
        for(int k_iter = 0; k_iter < Ktile; ++k_iter){
          ...
          Cout += a_thread_tile_offset_compute[k_iter] * b_thread_tile_offset_compute[k_iter * Ntile];
        }
        
        // Write back the result to the output matrix.
        *c_thread_tile_offset_compute = Cout;
      }
    }
    __syncthreads();
  }
