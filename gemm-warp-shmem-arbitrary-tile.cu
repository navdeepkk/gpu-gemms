...
#define WarpMtile 64  // This will actually be the loop step of `i` loop.
#define WarpNtile 64  // This will actually be the loop step of `j` loop.
#define WarpKtile 32  // This will actually be the loop step of `k` loop.
#define WarpSize 32
#define WarpdimX 4
#define WarpdimY 8
#define WarpdimZ 1
...
//} WarpDim;
//
//__device__ void initWarpShape(int x, int y, int z, WarpDim &warpDim){
//  warpDim.x = x;
//  warpDim.y = y;
//  warpDim.z = z;
//}
...
__global__ void GEMM(DTYPE * a, DTYPE * b, DTYPE * c, int m, int n, int k){
  ...
  constexpr int numWarpsInN = Ntile / WarpNtile;
  constexpr int numWarpsInM = Mtile / WarpMtile;
  ...
  int i_iter_warp_base = warpIdx.y * WarpMtile;
  int j_iter_warp_base = warpIdx.x * WarpNtile;
  ...
  int i_iter_thread_base = warpLocalTidY * Mchunk;
  int j_iter_thread_base = warpLocalTidX * Nchunk;
  ...
  DTYPE *c_warp_tile_offset = c_warp_tile_base + i_iter_warp_base * n + j_iter_warp_base;
  DTYPE *a_warp_tile_offset_compute;
  DTYPE *b_warp_tile_offset_compute;
  DTYPE *a_warp_tile_offset_copy;
  DTYPE *b_warp_tile_offset_copy;

  DTYPE *c_thread_tile_base = c_warp_tile_offset;
...
#pragma unroll
    ...
    // tiled for the warp. Tiling for warp may result is reduced register pressure
...
#pragma unroll
		...
		Cout[ci * Nchunk + cj] += a_thread_tile_offset_compute[i_iter * Ktile + k_iter] *1;//   b_thread_tile_offset_compute[k_iter * Ntile + j_iter];
	      }
	    }
	  }
	}
...
int main(){
  ...
  dim3 block((Mtile / WarpMtile) * (Ntile / WarpNtile) * WarpSize, 1, 1);
