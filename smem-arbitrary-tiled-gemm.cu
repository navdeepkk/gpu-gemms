...
__global__ void GEMM(DTYPE * a, DTYPE * b, DTYPE * c, int m, int n, int k){
  ...
  // Ussually ktile is less than mtile or ntile. Currently handeling the case only when ktile is
  ...
  for(int kk = 0; kk < (k + Ktile - 1)/Ktile; ++kk){
    for(int i = 0; i < (Ktile + 32 - 1) / 32; ++i){
      if((threadIdx.x + i) < (unsigned int) min_k_n){
	asmem[threadIdx.y][threadIdx.x + i] = a[(row * k) + (kk * Ktile) + threadIdx.x + i];			
      }
    }
    for(int i = 0; i < (Ktile + 32 - 1) / 32; ++i){
